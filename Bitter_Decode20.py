...
async def findb64(interaction: discord.Interaction, message: discord.Message):
    ...
    if counter + 1 > botreacted:
        for counter in range(botreacted, counter + 1):  # Add new reactions.
            try:
                await message.add_reaction(str(counter) + emoji[1:])
            except discord.errors.Forbidden: # change this
                try:
                    discord_error = discord.Embed(title='Error:', description='Bitter was unable to add reactions.')
                    await interaction.user.send(embed=discord_error)
                except:
                    pass
    per_reaction = cache.file()
