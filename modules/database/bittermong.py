...
settings = {name: Encodings.complete[name]["default"] for name in Encodings.names}
...
class BitterDB:
    def __init__(self, cn_str: str, db_name: str):
        ...
        self.bitter_db["channels"].create_index(
            [("channel_id", pymongo.ASCENDING)], unique=True
        )
    ...
    def add_guilds(self, guilds: Sequence) -> None:
        ...
        new_docs = [
            {"guild_id": guild.id, "name": guild.name, "settings": settings}
            for guild in guilds
            if guild.id not in old_id
        ]
        if len(new_docs) == 0:
            return
    ...
    def add_guild(self, guild) -> None:
        self.bitter_db["guilds"].insert_one(
            {"guild_id": guild.id, "name": guild.name, "settings": settings}
        )

    def remove_guild(self, guild_id) -> None:
        self.bitter_db["guilds"].delete_one({"guild_id": guild_id})
    ...
    def get_guild_settings(self, guild_id) -> Settings:
        ...
        if guild_setting is None:
            return {
                key: Encodings.complete[key]["default"]
                for key in Encodings.complete.keys()
            }
    ...
    def add_channel(self, guild_id, channel_id) -> None:
        self.bitter_db["channels"].insert_one(
    ...
    def insert_content(
        ...
        self.bitter_db["messages"].update_one(
            ...
            {
                "$set": {"jump_url": jump_url, "message_id": message_id},
                "$setOnInsert": {
    ...
    def edit_content(
        ...
        result = self.bitter_db["messages"].update_one(
            ...
            {"$set": {"content": content, "jump_url": jump_url}},
        )
    ...
    def delete_content(
        self, guild_id, channel_id, message_id, content: List[str]
    ) -> None:
